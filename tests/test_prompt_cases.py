...
from damin_gambit.db import DbConfig, reset_db, seed_db
...
def test_prompt_case_rules_fallback(case: dict, client: TestClient, seeded_db: Path, monkeypatch):
    ...
    assert len(results) == expect["rows"]
    ...
    # validate total product range (API computes product over returned values)
